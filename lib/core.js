...
AccountsTemplates.configureRoute = function(route, options) {
  ...
  if (layoutType === "blaze-to-react" || layoutType === "react") {
    ...
    if (require) {
      ...
      throw new Error("useraccounts:flow-routing requires that your project includes react-mounter or (deprecated) kadira:react-layout package.");
    }
    ...
    if (layoutType === "react") {
      layoutRegions[contentRegion] = template; // The template should be a React element already
    }
  }
